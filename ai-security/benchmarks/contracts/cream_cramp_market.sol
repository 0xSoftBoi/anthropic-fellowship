// Cream Finance crAMP market (Compound-fork CErc20) — verified, Ethereum
// Address: 0x2Db6c82CE72C8d7D770ba1b5F5Ed0b6E075066d6 (CErc20Delegator)
// Exploit 2021-08-30 (~$18.8M): AMP is ERC-777; tokensReceived hook re-enters borrow() before
// debt state is written (CEI violation). CAVEAT: current on-chain impl is the POST-HACK patched
// CheckRepay delegate; the structural CErc20 borrow flow is present but the fix may mask the bug.
...
contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    function borrowFresh(
         ...
         * Put behind `borrowAllowed` for accruing potential COMP rewards.
         */
        if (borrowAmount == 0) {
    ...
    function repayBorrowFresh(
         ...
         * Put behind `repayBorrowAllowed` for accruing potential COMP rewards.
         */
        if (repayAmount == 0) {
            accountBorrows[borrower].interestIndex = borrowIndex;
